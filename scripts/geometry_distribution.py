...
def main():
    parser = argparse.ArgumentParser(
        description="headerが'frame, distance'となっているCSVファイルから、フタ閉めに失敗する確率を見積もる"
    )
    ...
    print(
        "file name\ttotal number of attempts\taverage of number of attempts before failure\t逆数"
    )
    print(
        f"{args.input}\t{len(distances)}\t{len(geo_dist)}\t{number_of_attempts_before_failure}\t{round(1/number_of_attempts_before_failure, 3)}"
    )
    """
